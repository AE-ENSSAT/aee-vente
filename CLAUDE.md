...
Comments earn their place or go. Write the **why**, never the what: no comment that
restates the code, the function name, or a type. Keep them **short** — one line by
default, and a block only for something genuinely expensive to rediscover (a platform
quirk, a contract the types don't carry, a trade-off that looks wrong until explained).

- Prefer a clearer name or a smaller function over a comment explaining a muddled one.
- JSDoc on an exported symbol only where it says more than the signature does.
- No banners, no changelogs in code ("was X, now Y"), no paraphrasing a linked doc.
- Deleting a comment your change made stale is part of the change. A wrong comment costs
...
- **Auth** is a Keycloak JWT; the API has no login endpoint of its own. Realm `app` at
  ...
  Valid post logout redirect URI — plus **`aeevente-dev://auth`** for the dev variant,
  which carries its own scheme so both builds can sit on one device without fighting over
  who receives the callback (the exact string is logged at start-up in dev). PKCE is
...
- iOS requires the `com.apple.developer.proximity-reader.payment.acceptance` entitlement
  (set in `app.config.js`, granted by Apple on the provisioning profile). It lives on the
  association's team, so a build signed under a personal one installs but refuses to launch.
